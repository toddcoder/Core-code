...
namespace Core.RegularExpressions
{
   public class Matcher : IEnumerable<Matcher.Match>
   {
      ...
      public Matched<Match[]> MatchAll(string input, string pattern, RegexOptions options)
      {
         try
         {
            return IsMatch(input, pattern, options) ? matches.Match() : noMatch<Match[]>();
         }
      ...
      public Matched<Match> MatchOne(string input, string pattern, RegexOptions options)
      {
         try
         {
            return IsMatch(input, pattern, options) ? matches[0].Match() : noMatch<Match>();
         }
         catch (Exception exception)
         {
            return failedMatch<Match>(exception);
      ...
      public virtual void Evaluate(string input, RegexPattern regexPattern) => Evaluate(input, regexPattern.Pattern, regexPattern.Options);
