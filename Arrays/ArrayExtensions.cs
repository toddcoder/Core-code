...
namespace Core.Arrays
{
   public static class ArrayExtensions
   {
      ...
      public static T[] RangeOf<T>(this T[] array, int start, int stop)
      {
         ...
         Assert(start.Between(0).Until(array.Length), "start value out of range");
         Assert(stop.Between(0).Until(array.Length), "stop value out of range");
      ...
      public static IMaybe<T> Of<T>(this T[] array, int index) => when(index.Between(0).Until(array.Length), () => array[index]);

      public static T First<T>(this T[] array, T defaultValue) => array.IsEmpty() ? defaultValue : array[0];

      public static IMaybe<T> First<T>(this T[] array) => when(array.IsNotEmpty(), () => array[0]);

      public static T Last<T>(this T[] array, T defaultValue) => array.IsEmpty() ? defaultValue : array[array.Length - 1];

      public static IMaybe<T> Last<T>(this T[] array) => when(array.IsNotEmpty(), () => array[array.Length - 1]);
