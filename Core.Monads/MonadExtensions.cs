...
namespace Standard.Types.Monads
{
	public static class MonadExtensions
	{
		...
		public static IResult<T> FirstOrFail<T>(this IEnumerable<T> enumerable) => AttemptFunctions.tryTo(() =>
		{
			var first = enumerable.FirstOrDefault();
			if (first.IsNull())
		...
		public static IMatched<T> FirstOrNotMatched<T>(this IEnumerable<T> enumerable, Func<T, bool> predicate)
		{
			try
			{
				...
				if (first.IsNull())
					return MonadFunctions.notMatched<T>();
				else if (first.Equals(default))
					return MonadFunctions.notMatched<T>();
				else
			...
			{
				return MonadFunctions.failedMatch<T>(exception);
			}
		}
		...
		{
			if (obj.IsNull())
				return MonadFunctions.none<Type>();
			else
			{
		...
		public static IResult<TResult> Select<T, TResult>(this IResult<T> result, Func<T, TResult> func)
		{
			if (result.If(out var value))
				...
				return MonadFunctions.failure<TResult>(result.Exception);
		}
		...
		public static IResult<T> Failure<T>(this string message) => MonadFunctions.failure<T>(new Exception(message));
		...
		public static IResult<T> Tap<T>(this IResult<T> result, Action<IResult<T>> action) => AttemptFunctions.tryTo(() =>
		{
			action(result);
			return result;
		});
      ...
      public static (IEnumerable<T> enumerable, IMaybe<Exception> exception) SuccessesFirst<T>(this IEnumerable<IResult<T>> enumerable,
		...
		public static IResult<TResult> ForAny<TSource, TResult>(this IEnumerable<TSource> enumerable,
			Func<TSource, TResult> func) => AttemptFunctions.tryTo(() =>
		{
			var firstItem = MonadFunctions.none<TResult>();
			foreach (var result in enumerable.Select(item => AttemptFunctions.tryTo(() => func(item))))
		...
		public static IResult<TResult> ForAny<TSource, TResult>(this IEnumerable<TSource> enumerable,
			Action<TSource> action, TResult result) => AttemptFunctions.tryTo(() =>
		{
			foreach (var item in enumerable)
				...
				{
					return MonadFunctions.failure<TResult>(exception);
				}
		...
		public static IResult<TResult> ForAny<TSource, TResult>(this IEnumerable<TSource> enumerable,
			Action<TSource> action, Func<TResult> result) => AttemptFunctions.tryTo(() =>
		{
			foreach (var item in enumerable)
				try
				{
		...
		public static IResult<T> Flat<T>(this IResult<IResult<T>> result) => result.FlatMap(r => r, MonadFunctions.failure<T>);
		...
		public static IMaybe<T> IfCast<T>(this object obj) => obj is T t ? t.Some() : MonadFunctions.none<T>();
